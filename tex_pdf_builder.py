...
class LatexBuilder:
    ...
    def _candidate_paths_for_engine(self, engine: str) -> List[Path]:
        ...
        if texlive_root.exists() and texlive_root.is_dir():
            for path_str in sorted(texlive_root.glob("*/bin/*")):
                if path_str.is_dir():
    ...
    def compile_file(
        ...
        if not success and attempts:
            ...
            return_code = attempts[-1].return_code
        return FileResult(
...
def human_summary(results: List[FileResult], dry_run: bool = False) -> str:
    lines = []
    success_count = sum(1 for r in results if r.success)
    total = len(results)
    prefix = "DRY RUN - compilation not executed" if dry_run else "Build results"
    lines.append(f"{prefix}: {success_count}/{total} succeeded")
    for res in results:
        status = "OK" if res.success else "FAIL"
        engine = res.engine_used or "-"
        lines.append(f" - {res.file_name}: {status} (engine: {engine})")
    if dry_run:
        lines.append("Dry-run performed: compilations were not executed.")
    return "\n".join(lines)
...
def main(argv: Optional[Sequence[str]] = None) -> int:
        ...
        for tex in tex_files:
            results.append(
                FileResult(
                    ...
                    engine_used=None,
                    return_code=None,
                    stderr_excerpt="DRY_RUN: compilation not executed",
                    attempts=[],
                )
            )
    else:
    ...
    print(human_summary(results, args.dry_run))
